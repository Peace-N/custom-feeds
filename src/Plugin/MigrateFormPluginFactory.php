...
class MigrateFormPluginFactory {
  ...
  public function hasForm(PluginInspectionInterface $plugin, $operation) {
    ...
    return !empty($definition['customfeeds']['form'][$operation]);
  }
   ...
   * @param \Drupal\customfeeds\Entity\MigrationInterface|null $migration
  ...
  public function createInstance(PluginInspectionInterface $plugin, $operation, MigrationInterface $migration = NULL, array $configuration = []) {
    ...
    $form_plugin_id = $plugin_definition['customfeeds']['form'][$operation];
    if ($plugin instanceof PluginFormInterface && $plugin->getPluginId() === $form_plugin_id) {
      ...
      /* @var \Drupal\customfeeds\Plugin\MigrateFormPluginManager $manager */
      $manager = \Drupal::service("plugin.manager.customfeeds.migrate.{$form_plugin_type}_form");
      /** @var \Drupal\customfeeds\Plugin\MigrateFormPluginInterface $form_plugin */
