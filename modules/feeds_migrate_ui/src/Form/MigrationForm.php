...
/**
 * Provides a form for editing the base information of a migration.
 *
...
class MigrationForm extends EntityForm {
  ...
  protected function prepareEntity() {
    ...
    // schema.
    // First, see if there are query parameters to use.
    ...
    foreach ($params as $key => $param) {
      ...
      unset($params[$key]);
      $params[$safe_key] = $safe_param;
    }

    $default_plugin = 'url';

    if (array_key_exists('source_plugin', $params)) {
      // Only allow legitimate source plugins.
      $options = $this->getPluginOptionsList('source');
      $default_plugin = in_array($params['source_plugin'], $options) ? $params['source_plugin'] : 'url';
    }
    ...
    // If on the url a query parameter for 'migration_group' is set, check if a
    // migration group exists with that name. If so, set it as the entity's
    // migration group.
    $groups = MigrationGroup::loadMultiple();
    if (isset($params['migration_group']) && isset($groups[$params['migration_group']])) {
      $defaults['migration_group'] = $params['migration_group'];
    }
  ...
  protected function getPlugins() {
    ...
    // Source.
    $source = $this->entity->getSourceConfiguration();
    if (isset($source['plugin'])) {
      $plugins['source'] = $source['plugin'];
    }

    // Destination.
    $destination = $this->entity->getDestinationConfiguration();
